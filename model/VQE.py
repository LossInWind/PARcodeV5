...
class Encoder(nn.Module): #输入形状为(64,1,224,224) 输出为[64, 128, 56, 56]
    ...
    def __init__(self, in_channel, channel, n_res_block, n_res_channel, stride):
        ...
        blocks = [nn.Conv2d(in_channel, channel, kernel_size=(8, 8), stride=(4, 4), padding=(2, 2))]
...
class Quantizer(nn.Module):#输入形状为(32, 56, 56, 64)
    ...
    def forward(self, input):
        # 将输入展平为二维张量
        flatten = input.reshape(-1, self.dim) # [B, C, H, W] -> [B*H*W, C]
        # 计算每个向量和嵌入向量的欧氏距离
        dist = (
...
class VQE(nn.Module):
    ...
    def encode(self, input):
        ...
        # print(enc_b.shape)
        quant_b = self.quantize_conv_b(enc_b).permute(0, 2, 3, 1)  # 量化操作
